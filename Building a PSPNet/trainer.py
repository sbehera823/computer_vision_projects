...
def main_worker_wandb(args, use_cuda: bool):
    """ """
    model, optimizer = get_model_and_optimizer(args)
    logger.info(args)
    logger.info("=> creating model ...")
...
def run_epoch(
    ...
    for i, (input, target) in enumerate(data_loader):
        ...
        main_loss = torch.mean(main_loss) if main_loss is not None else torch.Tensor([0])
        aux_loss = torch.mean(aux_loss) if aux_loss is not None else torch.Tensor([0])
