...
class PSPNet(nn.Module):
    ...
    def __init__(
        ...
        self.cls = None
        self.aux = None
        base_resnet = resnet50(pretrained=pretrained, deep_base=deep_base)
        ...
        if self.use_ppm:
            ...
            self.ppm = PPM(2048, reduction_dim, bins)
            fea_dim += reduction_dim * len(bins)
    ...
    def __replace_conv_with_dilated_conv(self):
        ...
        for name, module in self.layer3.named_modules():
            if 'conv2' in name and isinstance(module, nn.Conv2d):
                ...
                module.dilation = (2, 2)
                module.padding = (2, 2)
            if 'downsample.0' in name and isinstance(module, nn.Conv2d):
                module.stride = (1, 1)  
    ...
    def __create_classifier(self, in_feats: int, out_feats: int, num_classes: int) -> nn.Module:
        ...
        cls = nn.Sequential(
            nn.Conv2d(in_feats, out_feats, 3, 1),
    ...
    def forward(
        ...
        output_size = (int(math.ceil(H / 8 * self.zoom_factor)), int(math.ceil(W / 8 * self.zoom_factor)))
        
        main_logits = self.cls(x)
        main_logits_upsampled = F.interpolate(main_logits, size=output_size, mode='bilinear', align_corners=True)
        
        aux_loss = None
        if self.training and hasattr(self, 'aux'):
            aux_logits = self.aux(x_aux)
            aux_logits_upsampled = F.interpolate(aux_logits, size=output_size, mode='bilinear', align_corners=True)
            if y is not None:
                aux_loss = self.criterion(aux_logits_upsampled, y)

        main_loss = None
        if y is not None and self.training:
            main_loss = self.criterion(main_logits_upsampled, y)

        yhat = torch.argmax(main_logits_upsampled, dim=1)

        return main_logits_upsampled, yhat, main_loss, aux_loss
