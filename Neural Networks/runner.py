...
from src.vision.dl_utils import compute_accuracy, compute_loss
...
class Trainer:
    ...
    def run_wandb_training_loop(self, num_epochs: int) -> None:
        """Train for num_epochs, and validate after every epoch. 
        Log the metrics of every epoch to WandB."""
        ############################################################################
        # Student code begin
        ############################################################################
        
        # 1. Call wandb.login(...) and use your API key.
        # 2. Call wandb.init(...) to initialize the run.
        # 3. Call wandb.log(...) after every epoch to log the epochwise metrics.
        # 4. Call wandb.finish() after the run is over.

        wandb.login(key="22d5de6bb7624aa3c22c66039f0f2aefc5910214")

        wandb.init(project="project", entity="sauravs-behera")
