...
def generate_confusion_data(
    ...
    class_labels = list(dataset.class_dict.keys())
...
def generate_accuracy_data(
    ...
    return all_targets, all_preds, class_labels
...
def generate_accuracy_table(
    ...
    correct_preds = np.equal(targets, preds).astype(np.float32)
    accuracy_per_attribute = correct_preds.mean(axis=0)
