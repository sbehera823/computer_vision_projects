...
class ImageLoader(data.Dataset):
        ...
        for class_name, class_index in class_labels.items():
            ...
            for img_file in os.listdir(class_dir):
                ...
                if img_path.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif')):
                    img_paths.append((img_path, class_index))
    ...
    def get_classes(self) -> Dict[str, int]:
        ...
        items = os.listdir(self.curr_folder)
        class_dirs = [item for item in items if os.path.isdir(os.path.join(self.curr_folder, item))]
        class_dirs.sort()
        classes = {class_name: index for index, class_name in enumerate(class_dirs)}
        return classes
