...
def compute_image_gradients(image_bw: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    image_tensor = torch.from_numpy(image_bw).float().unsqueeze(0).unsqueeze(0)  # Shape: [1, 1, M, N]
    sobel_x_kernel = torch.from_numpy(SOBEL_X_KERNEL).unsqueeze(0).unsqueeze(0)  # Shape: [1, 1, 3, 3]
    sobel_y_kernel = torch.from_numpy(SOBEL_Y_KERNEL).unsqueeze(0).unsqueeze(0)  # Shape: [1, 1, 3, 3]
    ...
    conv_x.weight = nn.Parameter(sobel_x_kernel, requires_grad=False)
    conv_y.weight = nn.Parameter(sobel_y_kernel, requires_grad=False)
    Ix_tensor = conv_x(image_tensor)
    Iy_tensor = conv_y(image_tensor)
    Ix = Ix_tensor.squeeze().detach().numpy()
    Iy = Iy_tensor.squeeze().detach().numpy()
...
def nms_maxpool_pytorch(
    ...
    median_val = torch.median(R_tensor)
    R_thresholded = torch.where(R_tensor < median_val, torch.tensor(0.0), R_tensor)
    maxpool = nn.MaxPool2d(ksize, stride=1, padding=(ksize - 1) // 2, return_indices=False)
    R_maxpooled = maxpool(R_thresholded.unsqueeze(0).unsqueeze(0)).squeeze()
    mask_local_maxima = R_maxpooled == R_thresholded
    R_filtered = R_thresholded * mask_local_maxima
    c, indices = torch.topk(R_filtered.view(-1), k)
    y, x = torch.div(indices, R_filtered.shape[1], rounding_mode='trunc'), indices % R_filtered.shape[1]
    x = x.numpy()
    y = y.numpy()
    c = c.numpy()
    return x, y, c
...
def get_harris_interest_points(
    ...
    x, y, c = nms_maxpool_pytorch(R_normalized, k=k, ksize=7)
