...
def compute_feature_distances(
    ...
    # f1_expanded = np.expand_dims(features1, axis=1) 
    # f2_expanded = np.expand_dims(features2, axis=0)  
    # dists = np.sqrt(np.sum((f1_expanded - f2_expanded) ** 2, axis=2))
    
    # return dists

    sum_square_features1 = np.sum(np.square(features1), axis=1, keepdims=True)
    sum_square_features2 = np.sum(np.square(features2), axis=1, keepdims=True)
    dists_squared = sum_square_features1 + sum_square_features2.T - 2 * features1 @ features2.T
...
def match_features_ratio_test(
    ...
    nn_indices = sorted_indices[:, :2]
    ratios = dists[np.arange(dists.shape[0]), nn_indices[:, 0]] / dists[np.arange(dists.shape[0]), nn_indices[:, 1]]
    ratio_test_threshold = 0.8
    valid_matches = ratios < ratio_test_threshold
    features1_indices = np.arange(dists.shape[0])[valid_matches]
    features2_indices = nn_indices[valid_matches, 0]
    matches = np.vstack((features1_indices, features2_indices)).T
    confidences = 1 / dists[features1_indices, features2_indices]
